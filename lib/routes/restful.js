...
var CRUD = {
    ...
    update: function(service) {
        var params = service.request.params;
        var body = service.request.body  
        commondb
            .updateById(service.collection, params.id, body)
            .then((results) => response(service, 200, results))
            .catch(util.propagateError(service.next));
    },
...
function routing(router, collection) {
    ...
    router.put('/:id', readParams(CRUD.update, collection));
